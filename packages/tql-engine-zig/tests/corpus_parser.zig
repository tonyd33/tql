...
pub const TestCase = struct {
    ...
    pub fn deinit(self: *TestCase, allocator: std.mem.Allocator) void {
        ...
        if (self.expected_ast) |s| allocator.free(s);
...
fn parseCase(
    ...
    const target = try extractSection(allocator, lines, SECTION_SOURCE, SECTION_AST);
    errdefer allocator.free(target);

    const expected_ast_raw = try extractSection(allocator, lines, SECTION_AST, SECTION_BYTECODE);
    errdefer allocator.free(expected_ast_raw);
    const expected_ast: ?[]const u8 = if (expected_ast_raw.len == 0) blk: {
        allocator.free(expected_ast_raw);
        break :blk null;
    } else expected_ast_raw;
...
fn extractSection(
    ...
    while (lines.next()) |line| {
        if (std.mem.eql(u8, line, end_marker)) break;
